    ...
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "frame": {
                    "type": "integer",
                    "description": "seek_capture/watchで移動するフレーム番号",
                },
                "duration_ms": {
                    "type": "integer",
                    "description": "record/watchで録音する時間(ms)、デフォルト3000/5000",
                },
                "capture_interval_ms": {
                    "type": "integer",
                    "description": "watchでフレームをキャプチャする間隔(ms)、デフォルト1000",
                },
                "element": {
                    "type": "string",
                    "description": "キャプチャ対象のWPF要素名(省略可)",
                },
            },
            "required": ["action"],
        },
...
async def dispatch(args: dict) -> Any:
    ...
    match action:
        case "get_info":
            match sub_action:
                case "status":
                    return await ymm4_get("/status")
                case "project":
                    return await ymm4_get("/project")
                case "items":
                    return await ymm4_get("/items")
                case "effects_list":
                    return await ymm4_get("/effects/list")
                case _:
                    raise ValueError(f"Unknown sub_action for get_info: {sub_action}")

        case "control":
            match sub_action:
                case "play":
                    return await ymm4_post("/playback/play")
                case "stop":
                    return await ymm4_post("/playback/stop")
                case "save":
                    return await ymm4_post("/project/save")
                case _:
                    raise ValueError(f"Unknown sub_action for control: {sub_action}")
        ...
        case "add_item":
            ...
            match sub_action:
                case "text":
                    return await ymm4_post("/items/text", payload)
                case "voice":
                    return await ymm4_post("/items/voice", payload)
                case "tachie":
                    return await ymm4_post("/items/tachie", payload)
                case "face":
                    return await ymm4_post("/items/face", payload)
                case _:
                    raise ValueError(f"Unknown sub_action for add_item: {sub_action}")
        ...
        case "edit_item":
            match sub_action:
                ...
                case "delete":
                    payload = {}
                    if "frame" in args and args["frame"] != -1:
                        payload["frame"] = args["frame"]
                ...
                case "duration":
                    return await ymm4_post(
                        "/timeline/duration", {"frames": args.get("frames", 0)}
                    )
                case _:
                    raise ValueError(f"Unknown sub_action for edit_item: {sub_action}")
        ...
        case "watch":
            ...
            if not data.get("success"):
                return CallToolResult(
                    content=[
                        TextContent(
                            type="text", text=f"❌ {data.get('error', 'unknown error')}"
                        )
                    ],
                    isError=True,
                )
            ...
            if audio_b64:
                ...
                wav_bytes = base64.b64decode(audio_b64)
                save_dir = os.path.dirname(os.path.abspath(__file__))
                wav_path = os.path.normpath(
                    os.path.join(save_dir, "..", "watch_result.wav")
                )
                with open(wav_path, "wb") as f:
                    f.write(wav_bytes)
                contents.append(
                    TextContent(type="text", text=f"💾 音声保存: {wav_path}")
                )
            # 各フレーム画像
...
def _preview_result(data: dict) -> CallToolResult:
    ...
    if not image_b64:
        return CallToolResult(
            content=[TextContent(type="text", text=format_result(data))], isError=True
        )
